...
요약 후에는 해당 문서를 기반으로 **후속 질문(문서 기반 대화형 QA)** 을 이어갈 수 있으며, 결과는 txt / docx / pdf로 다운로드할 수 있습니다.
회원가입/로그인 후 모든 요약 이력과 다운로드 이력이 Oracle DB에 저장되고, 마이페이지에서 확인할 수 있습니다.

> 단순 요약 시스템에서 **문서 기반 대화형 QA**로 확장 중입니다. 현재는 외부 웹 검색 없이, 문서 원문과 요약을 컨텍스트로 삼아 자연스러운 문서 Q&A를 지원합니다.
...
| 문서 기반 대화 | 완료 | 원문 기반 Q&A, strict/chat 모드, 전제 검증, 3층위 답변 구조 |
| 세션 복원 | 완료 | 새로고침 시 요약 결과·대화 내역 localStorage 복원 |
| 한국어 출력 보정 | 완료 | 요약·채팅 모두 CJK 후처리 구현 (모델 출력 품질에 따라 간헐 발생 가능) |
...
ChatPanel 헤더에서 두 가지 모드를 전환할 수 있습니다.

| 모드 | 동작 |
|------|------|
| **대화형** (기본) | 문서 근거 우선. 부족하면 "문서를 바탕으로 해석하면" / "일반적으로는" 표현으로 층위 구분해 보충 |
| **엄격** | 문서에 있는 내용만 답변. 근거 없으면 "문서에서 확인할 수 없습니다" |

#### 답변 3층위 구조 (대화형 모드)

| 층위 | 표현 방식 |
|------|----------|
| ① 문서에서 직접 확인된 사실 | 수식어 없이 서술 |
| ② 문서 기반 추론/해석 | "문서를 바탕으로 해석하면", "정황상" |
| ③ 일반 배경지식 (보조) | "문서에는 구체적인 내용이 언급되지 않습니다. 일반적으로는…" (1~2문장 이내) |

#### 전제 검증

질문에 문서와 다른 전제가 포함된 경우, 그 전제를 먼저 교정해서 답합니다.

> 예: 문서에 '공격 위협'만 있는데 "왜 공격했지?"라고 물으면
> → "문서에서는 실제 공격이 아니라 공격 위협/준비 단계로 기술되어 있습니다."

#### 컨텍스트 선택 전략 (`chat_service.py`)

긴 문서를 매번 통째로 투입하지 않고, 질문과 관련된 구절만 선별해 사용합니다.
원문이 요약보다 검증 우선 기준이 됩니다.

| 조건 | 사용 컨텍스트 |
|------|--------------|
| `input_text` 없음 (구버전 기록) | `[문서 요약]` fallback |
| 원문 50자 미만 (OCR 노이즈) | `[문서 요약]` fallback |
| 원문 6,000자 이하 | `[원문 — 검증 기준]` + `[요약 — 참고용]` |
| 원문 6,000자 초과 | `[원문 관련 구절 — 검증 기준]` (키워드 매칭) + `[요약 — 참고용]` |

#### CJK 후처리

모델 응답에 중국어/한자가 섞인 경우:
- **자동 후처리**: regex 기반 CJK 제거(`_strip_cjk`) — Ollama 재호출 없이 즉시 처리
- **"한국어로 다시 말해줘" 요청**: 전용 재작성 경로로 처리하며, 결과 검증 후 이상 시 regex fallback
...
│   │       └── ...
│   ├── tests/
│   │   └── test_chat_service.py     # chat_service 단위 테스트 (34개)
│   ├── requirements.txt
│   └── .env
│
└── frontend/
...
### 2026-03-27 — 문서 기반 채팅 QA 개선 및 후처리 안정화

**채팅 기능 개선**

- **strict / chat 응답 모드 분리** — ChatPanel 헤더 토글로 전환. chat(기본)은 3층위 답변 구조 허용, strict는 문서 근거만 답변
- **전제 검증 로직 추가** — 질문에 문서와 다른 전제가 있으면 그 전제를 먼저 교정하고 답변 (예: 위협을 공격으로 오인한 질문)
- **원문 우선 검증 기준** — 컨텍스트 블록 순서를 "원문 → 요약" 순으로 통일. 요약보다 원문을 검증 기준으로 사용
- **일반론 확장 제한** — 문서에 없는 내용은 "문서에는 구체적인 내용이 언급되지 않습니다"를 먼저 밝히고, 일반 설명은 1~2문장 이내로 제한

**CJK 혼용 후처리 안정화**

- **자동 후처리 분리** — CJK 감지 시 Ollama 재호출 대신 `_strip_cjk()`(regex)로만 처리해 지시문 노출 버그 제거
- **"한국어로 다시 말해줘" 전용 경로** — 사용자 명시 요청은 별도 재작성 프롬프트 + 결과 검증 후 이상 시 regex fallback
- **`_is_instruction_leak()` 방어 로직** — 재작성 결과에 지시문 패턴이 감지되면 안전한 결과로 대체
- **디버그 로그** — 원본 응답·CJK 감지·strip 전후 길이·재작성 입출력을 DEBUG 레벨로 기록

**테스트**

- `backend/tests/test_chat_service.py` 추가 — 34개 단위 테스트 전체 통과
  - CJK 감지/제거, 지시문 노출 감지, 재작성 요청 판별, 이전 대화 추출
  - 회귀 케이스: "자임추모공원의 위치는?", "언제부터 시작됐어?", "한국어로 다시 말해줘"
...
| 항목 | 상태 |
|---|---|
| 채팅 한국어 보정 | regex 기반 CJK 후처리 구현 완료. 완전 종결은 아니며 모델 출력 품질에 따라 간헐 발생 가능 |
...
1. **파일 변환 서비스** — 대시보드에 이미 카드로 예고됨, 다음 서비스로 구현 예정
2. **채팅 이력 DB 저장** — `CHAT_SESSIONS` / `CHAT_MESSAGES` 테이블로 서버 영속화 (현재 localStorage만)
3. **임베딩 기반 문단 검색** — `_select_relevant_paragraphs()`를 벡터 유사도 검색으로 교체 (교체 포인트 설계 완료)
4. **채팅 CJK 후처리 고도화** — regex 제거 후 문장 흐름이 어색한 경우 추가 보정
